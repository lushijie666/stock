...
def calculate_all_signals_by_strategy(df: pd.DataFrame, strategies: List[StrategyType] = None) -> Dict[
    StrategyType, StrategyResult]:
    """
    根据指定策略计算信号
    ...
    for strategy_type in strategies:
        if strategy_type in strategy_map:
            ...
            results[strategy_type] = strategy.generate_signals(df)
...
def calculate_all_signals(df: pd.DataFrame, strategies: List[StrategyType] = None,
                                merge_and_filter: bool = True) -> List[Dict]:
    """
       计算所有策略的信号并合并为一个信号列表
...
def calculate_all_signals_by_strategy(df: pd.DataFrame, strategies: List[StrategyType] = None,
                                      merge_and_filter: bool = True) -> Dict[StrategyType, StrategyResult]:
    """
    根据指定策略计算信号
